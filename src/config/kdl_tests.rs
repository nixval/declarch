...
use crate::config::kdl_modules::parse_kdl_content;
    ...
    fn test_inline_prefix() {
        ...
        assert!(config.packages_by_backend.get("paru").unwrap().iter().any(|p| p.name == "hyprland"));
        assert!(config.packages_by_backend.get("npm").unwrap().iter().any(|p| p.name == "typescript"));
        assert!(config.packages_by_backend.get("flatpak").unwrap().iter().any(|p| p.name == "com.spotify.Client"));
    }
