...
/// ```
pub fn format_error_report(content: &str, file_path: Option<&str>, error: &KdlError) -> String {
    ...
    if diagnostics.is_empty() {
        ...
        let _ = writeln!(
            &mut report,
            "\x1b[31merror\x1b[0m: Failed to parse KDL document"
        );
        return report;
    }
    ...
    for (i, diag) in diagnostics.iter().enumerate() {
        ...
        if let Some(help) = &diag.help {
            let _ = writeln!(
                &mut report,
                "  \x1b[34m=\x1b[0m \x1b[33mhelp\x1b[0m: {}",
                help
            );
        } else {
...
fn get_error_hint(content: &str, error_line: usize, message: &str) -> Option<String> {
    ...
    if message.contains("unexpected token") || message.contains("bare word") {
        ...
        return Some(
            "\x1b[33mhint\x1b[0m: Check for missing quotes around values or invalid keywords"
                .to_string(),
        );
    }
...
) -> Result<kdl::KdlDocument, String> {
    content
        .parse::<kdl::KdlDocument>()
        .map_err(|e: kdl::KdlError| format_error_report(content, file_path, &e))
}
